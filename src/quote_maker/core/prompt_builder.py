...
- `markup`: non-negative fraction (e.g. `0.30` for 30%).
...
- The renderer computes each item's amount as `qty * unit_cost * contract`, then sums by
  section, applies `markup` to produce a pre-tax total, and applies `tax` for the grand total.
